...
function Weather() {
  ...
  // Список ISO-кодов стран Центральной Азии и Турции
  const centralAsianCountries = ["KZ", "UZ", "KG", "TJ", "TR", "MN"];

  useEffect(() => {
    // Фильтруем страны по их ISO-кодам
    setAllCountries(
      Country.getAllCountries()
        .filter((country) => centralAsianCountries.includes(country.isoCode)) // Фильтрация
        .map((country) => ({
          value: {
            name: country.name,
